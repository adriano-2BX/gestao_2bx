...
def log_llm_usage(usage: schemas.LLMCustoUsoCreate, db: Session = Depends(get_db), user: models.Usuario = Depends(require_permission("llm_custo:criar"))):
...
def read_llm_custos(projeto_id: Optional[int] = None, db: Session = Depends(get_db), user: models.Usuario = Depends(require_permission("llm_custo:ver"))):
